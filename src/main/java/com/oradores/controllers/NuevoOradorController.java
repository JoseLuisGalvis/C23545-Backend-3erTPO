...
import com.oradores.repository.MySQLOradorRepository;
import com.oradores.repository.OradorRepository;
...
public class NuevoOradorController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, // aca viene todos lo del front
		...
		String json = request.getReader().lines().collect(Collectors.joining(System.lineSeparator()));// spring

		// convierto de json String a Objecto java usando libreria de jackson2
		ObjectMapper mapper = new ObjectMapper();
		mapper.registerModule(new JavaTimeModule());
		mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
		// Enlace con la Clase OradorRequest
		OradorRequest oradorRequest = mapper.readValue(json, OradorRequest.class);

		// grabamos en db
		Orador orador = new Orador(oradorRequest.getNombre(), oradorRequest.getApellido(),
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// escribe la respueta en el objeto response (que despues es lo que recibe el
		// front)
